...
const unsigned long heartbeatInterval = 900000; // 15 minutes (900,000 ms)
...
// #define DEVICE_SERIAL_NUMBER "24000003"
...
bool hasDataChanged() {
  ...
  bool binaryChanged = (doorOpen != prevDoorOpen) ||
                      (waterLeakage != prevWaterLeakage) ||
                      (acPowerFailure != prevAcPowerFailure);
...
void sendDataToServer(bool forceSend) {
  ...
  String jsonData = "{\"serialNumber\":\"" + String(device_serial_number) + 
                   "\",\"humidity\":\"" + String(humidity) + 
                   "\",\"temperature\":\"" + String(temperature) + 
                   "\",\"doorOpen\":\"" + String(doorOpen) + 
                   "\",\"waterLeakage\":\"" + String(waterLeakage) + 
